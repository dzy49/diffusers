...
class StableDiffusion3Pipeline_pos(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    r"""
    Args:
        transformer ([`SD3Transformer2DModel`]):
            Conditional Transformer (MMDiT) architecture to denoise the encoded image latents.
    ...
    def _get_cam_pos_embeds(self, yaw, pitch, lx, ly, lz, l_intensity, l_temp, focal_len, emotion,  device: Optional[torch.device] = None,):
        device = device or self._execution_device
    ...
    def encode_prompt(
        ...
        yaw = None,
        pitch = None,
        lx= None, ly= None, lz= None, l_intensity= None, l_temp= None, focal_len= None, emotion= None,
    ):  
        r"""
        ...
        if prompt_embeds is None:
            ...
            #print(yaw)
            #print(pitch)
            cam_pos_embeds = self._get_cam_pos_embeds(yaw, pitch, 
                                                      lx, ly, lz, l_intensity, l_temp, 
                                                      focal_len, emotion, device=device)

            prompt_embeds = torch.cat([clip_prompt_embeds, t5_prompt_embed], dim=-2)
            pooled_prompt_embeds = torch.cat([pooled_prompt_embed, pooled_prompt_2_embed], dim=-1)
            #pooled_prompt_embeds = pooled_prompt_embeds + cam_pos_embeds
            print("pooled before", pooled_prompt_embeds)
            print(torch.mean(cam_pos_embeds, dim=-2))
            pooled_prompt_embeds =  torch.mean(cam_pos_embeds, dim=-2) + pooled_prompt_embeds
            cam_pos_embeds = torch.nn.functional.pad(cam_pos_embeds, (0, 4096 - cam_pos_embeds.shape[-1]))
            #print(cam_pos_embeds.shape)
            #print(prompt_embeds.shape)
            prompt_embeds = torch.cat((cam_pos_embeds, prompt_embeds),dim=-2)
            
            print(torch.mean(cam_pos_embeds, dim=-2)[0][-1])
            print("pooled after", pooled_prompt_embeds)
    ...
    def __call__(
        ...
        (
            ...
            yaw=yaw,
            pitch=pitch,
            lx= lx, ly= ly, lz= lz, l_intensity= l_intensity, l_temp= l_temp, focal_len= focal_len, emotion= emotion,
        )
